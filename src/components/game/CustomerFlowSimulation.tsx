...
type TokenType = 'walkin' | 'cnc' | 'courier';
type TokenState =
  | 'entering'
  | 'deciding'
  | 'queuing'
...
type StaffAnimState = 'idle' | 'serving' | 'traveling-to-machine' | 'making' | 'traveling-to-till' | 'wandering';
...
function deriveStaffConfig(flags: ActionFlags): StaffConfig {
...
function deriveSimRates(metrics: VisibleMetrics, flags: ActionFlags): SimRates {
...
function initStaffTokens(config: StaffConfig): StaffToken[] {
...
function createInitialState(rates: SimRates): SimState {
  return {
    tick: 0,
    tokens: [],
...
function tickSimulation(state: SimState): SimState {
  const { rates } = state;
...
export default function CustomerFlowSimulation({ metrics, flags, triggerKey }: Props) {
  ...
  if (state.tick >= MIN_DISPLAY_TICKS) {
    ...
    const recentCount = completedTicks.filter(t => t > state.tick - SIM_HOUR).length;
    ordersPerHourText = ` | Orders/hr: ${Math.round(recentCount * SIM_HOUR / windowTicks)}`;
    const recentLost = lostTicks.filter(t => t > state.tick - SIM_HOUR).length;
    ...
    <div className="w-full bg-white rounded-xl border border-border-default shadow-md overflow-hidden">
      ...
      >
        ...
        <text x={VB_W - 10} y={22} textAnchor="end" fontSize={20}
          ...
          Queue: {tokens.filter(t => t.state === 'queuing').length} |
          Waiting: {tokens.filter(t => t.state === 'waiting').length} |
